...
use App\Exports\DropoutFeeStatusExport;
use Maatwebsite\Excel\Facades\Excel;


class DropoutFeeStatusController extends Controller
{
    protected string $permissionPrefix = 'dropout_fee_status';
    ...
    {   
        ...
        $query = Student::onlyTrashed(['collegeData'])
            ->select(
                'id',
                'student_name',
                'sno',
...
if ($request->filled('percent_range')) {
    ...
    switch ($request->percent_range) {
        ...
        return view('dropout-fee-status.index', compact(
            'students',
            'colleges',
            'courses',
            'totalFee',
    ...
    public function export(Request $request)
    {
         $date = strtolower(now()->format('d_F'));
          $parts = ['dropout_fee_status'];
         $fileName = implode('_', $parts) . '_' . $date . '.xlsx';
        return Excel::download(
            new DropoutFeeStatusExport(
                $request->college_id,
                $request->course_id,
