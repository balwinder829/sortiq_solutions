...
<div class="container">
    ...
    <div class="row mb-2">
        <div class="col-md-6">
            <h1 class="page_heading">Company PPTs</h1>
        </div>
        <div class="col-md-6">
            <div class="d-flex justify-content-end">
                <a href="{{ route('company_ppt.create') }}"
                   class="btn"
                   style="background-color:#6b51df;color:#fff;">
                    <i class="bx bx-plus"></i> Add Company PPT
                </a>
            </div>
        </div>
    </div>
    ...
    <div class="mb-3">
        ...
        <a href="{{ route('company_ppt.index',['filter'=>'active']) }}"
           class="btn btn-sm btn-outline-success {{ request('filter')=='active' ? 'active' : '' }}">
            Active
        </a>

        <a href="{{ route('company_ppt.index',['filter'=>'expired']) }}"
                ...
                @foreach($companyPpts as $cp)
                    <tr>
                        ...
                        <td class="no-wrap">
                            ...
                            <a href="{{ route('company_ppt.admin.download', $cp->id) }}"
                               class="btn btn-sm btn-success">
                                <i class="fa fa-download"></i>
                            </a>

                            <a href="{{ route('company_ppt.edit', $cp->id) }}"
                               class="btn btn-sm btn-outline-secondary">
                                <i class="fa fa-edit"></i>
                            </a>

                            <form action="{{ route('company_ppt.destroy', $cp->id) }}"
                                  method="POST"
                                  class="d-inline"
                                  onsubmit="return confirm('Delete this company PPT?');">
                                @csrf
                                @method('DELETE')
